...
const PositionSelectors = ({ containerType, containerNbr, onPositionChange }: { containerType: string, containerNbr?: string, onPositionChange: (pos: string) => void }) => {
    ...
    // Query Available Options - include containerNbr for restack validation
    const { data: termData, isLoading: isLoadingTerminals } = useQuery({
        queryKey: ['posInit', containerType, containerNbr],
        queryFn: () => yardApi.getAvailablePositionLov({ flag: 'I', containerType, containerNbr }),
        select: res => res.data
    });

    const { data: blockData, isLoading: isLoadingBlocks } = useQuery({
        queryKey: ['posBlock', terminal, containerNbr],
        queryFn: () => yardApi.getAvailablePositionLov({ flag: 'T', containerType, terminal, containerNbr }),
        enabled: !!terminal,
        select: res => res.data
    });

    const { data: lotData, isLoading: isLoadingLots } = useQuery({
        queryKey: ['posLot', block, containerNbr],
        queryFn: () => yardApi.getAvailablePositionLov({ flag: 'B', containerType, terminal, block, containerNbr }),
        enabled: !!block,
        select: res => res.data
    });

    const { data: rowData, isLoading: isLoadingRows } = useQuery({
        queryKey: ['posRow', lot, containerNbr],
        queryFn: () => yardApi.getAvailablePositionLov({ flag: 'L', containerType, terminal, block, lot, containerNbr }),
        enabled: !!lot,
        select: res => res.data
    });

    const { data: levelData, isLoading: isLoadingLevels } = useQuery({
        queryKey: ['posLevel', row, containerNbr],
        queryFn: () => yardApi.getAvailablePositionLov({ flag: 'R', containerType, terminal, block, lot, row, containerNbr }),
        enabled: !!row,
        select: res => res.data
    });
