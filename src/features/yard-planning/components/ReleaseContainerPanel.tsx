...
import { showRichToast } from '../../../components/ui/feedback/common/Toast';
import type { ToastDetailItem } from '../../../components/ui/feedback/common/Toast';
import { useStore } from '../../../store/store';
import {
    useTruckSuggestionsQuery,
    useReleaseContainerTruckDetailsQuery,
    useSubmitReleaseContainerMutation
} from '../apis/releaseContainerApi';
import type {
    SelectedContainer,
...
export default function ReleaseContainerPanel({ isOpen, onClose }: ReleaseContainerPanelProps) {
    ...
    const { data: allTrucks = [], isLoading: isLoadingTrucks, isError: isTrucksError, refetch: refetchTrucks } = useTruckSuggestionsQuery('', isOpen);
        ...
        <>
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px' }}>
                {isLoadingTrucks ? (
                    ...
                    <PremiumStateView
                        type="empty"
                        title={searchText ? "No Trucks Match" : "No Trucks Available"}
                        description={searchText ? `No trucks found matching "${searchText}"` : "There are currently no trucks available for release."}
                        icon={Truck}
                        height={300}
                        action={searchText ? {
                            label: "Clear Search",
