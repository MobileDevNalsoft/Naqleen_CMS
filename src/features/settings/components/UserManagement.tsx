...
import GenericEmptyState from '../../../components/ui/feedback/generic/GenericEmptyState';
        ...
        <div ref={containerRef} style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '10px', padding: '16px', position: 'relative' }}>
            ...
            {isLoading ? (
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                    <GenericEmptyState
                        type={searchQuery || selectedRoleFilters.length > 0 || selectedStatusFilter !== 'ALL' ? 'search' : 'data'}
                        title={searchQuery ? 'No users found' : 'No users registered'}
                        description={searchQuery ? 'Try adjusting your search or filters' : 'Get started by creating a new user.'}
                        icon={searchQuery ? SearchX : UserX}
                        action={!searchQuery ? { label: 'Create User', onClick: handleOpenAddUser } : undefined}
                    />
                </div>
            ) : (
                <div style={{ flex: 1, overflowY: 'auto' }}>
